...
8. 좁은 화면에서 지도 카드의 중첩 여백을 줄여 약도 가로 폭을 약 18% 넓혔다.
9. 장소 카드나 추천 흐름을 누르면 선택한 마커를 강조하고 약도가 화면 중앙 부근으로 이동하도록 했다.
10. 활성 마커에 은은한 확산 링과 별 반짝임을 추가했다. 모션 축소 설정에서는 반복 애니메이션이 자동으로 중단된다.
11. 장소 설명을 약도 아래의 전용 영역으로 분리해 PC와 모바일 모두 마커를 가리지 않도록 했다.
12. 자동 스크롤 중 포인터 아래를 지나간 다른 카드가 선택을 가로채지 않도록 호버 판정을 실제 포인터 이동 기준으로 안정화했다.
...
- Day 2 장소 카드 선택 후 약도 자동 이동 및 선택 마커 유지
- 모든 장소 목록에서 `본태박물관` 선택 후 약도 자동 이동 및 선택 유지
- PC 우도 약도의 6개 마커를 연속 선택해 각각 정확히 활성화되는지 확인
- 활성 마커의 `marker-glow`, `marker-twinkle` 애니메이션 적용 확인
- PC·모바일에서 장소 설명 영역이 약도 아래에 위치하고 지도 내부 설명창이 없는지 확인
- 모든 화면에서 가로 넘침 없음
- 브라우저 콘솔 경고·오류 없음
...
dist/assets/index-*.css        35.36 kB (gzip 8.32 kB)
dist/assets/index-*.js        217.48 kB (gzip 69.05 kB)
✓ built successfully
```
...
- `f9dbc47` 마커 설명 자동 배치와 약도 자동 스크롤 추가
- `ceecbd9` 모바일 지도 설명을 약도 아래로 분리
- `18c4fdb` 선택 마커 반짝임과 지도 이동 안정화
- `1b12299` 자동 스크롤 중 선택 상태 유지
- `d28445a` 모든 화면의 장소 설명을 약도 밖으로 분리
