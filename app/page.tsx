...
function PlaceIcon({kind}:{kind:Place["kind"]}){
  ...
  return <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" {...common}>
    {kind==="spot"&&<><path d="M2.5 19.5 9 9l3.5 5 2.5-3.5 6.5 9H2.5Z"/><circle cx="17.5" cy="5.5" r="2"/></>}
    {kind==="food"&&<><path d="M4 3v7a3 3 0 0 0 6 0V3M7 3v18M16 3c2.4 1.6 4 4.5 4 8v2h-4V3ZM18 13v8"/></>}
    {kind==="cafe"&&<><path d="M3 8h13v7a5 5 0 0 1-5 5H8a5 5 0 0 1-5-5V8ZM16 9h2a3 3 0 1 1 0 6h-2M5 4h9"/></>}
    {kind==="stay"&&<><path d="M3 20V5M3 17h18v3M3 12h18v5M7 12V8h5v4M12 12h7a2 2 0 0 1 2 2v3"/></>}
  </svg>;
}
...
const dayImages = [
  {src:"day-1-gimnyeong.jpg",alt:"김녕 바다의 노을을 표현한 이미지"},
  {src:"day-2-udo.jpg",alt:"우도의 맑은 바다를 표현한 이미지"},
  {src:"day-3-bijarim.jpg",alt:"비자림 숲길을 표현한 이미지"},
  {src:"day-4-hallasan.jpg",alt:"한라산 자락의 가을 풍경을 표현한 이미지"},
  {src:"day-5-west-coast.jpg",alt:"제주 서쪽 해안절벽의 노을을 표현한 이미지"},
  {src:"day-6-tangerines.jpg",alt:"제주 감귤밭을 표현한 이미지"},
  {src:"day-7-waterfall.jpg",alt:"제주 폭포 풍경을 표현한 이미지"},
];
  ...
  return <div className="map-card" aria-label={`${day.title} 약도`} ref={mapRef}>
    ...
    <div className="map-stage-shell"><div className={`map-stage ${isUdo?"udo-map":""} ${mapZoom.zoom>1?"zoomed":""}`} ref={mapZoom.viewportRef} onPointerDown={mapZoom.onPointerDown} onPointerMove={mapZoom.onPointerMove} onPointerUp={mapZoom.onPointerUp} onPointerCancel={mapZoom.onPointerCancel}>
      <div className="map-scroll-space" style={{width:`${mapZoom.zoom*100}%`,height:`${mapZoom.zoom*100}%`}}><div className="map-zoom-canvas" style={{width:`${100/mapZoom.zoom}%`,height:`${100/mapZoom.zoom}%`,transform:`scale(${mapZoom.zoom})`,"--map-marker-scale":1/mapZoom.zoom} as CSSProperties}>
        ...
        {places.map((place,index) => <button key={place.id} className={`map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected.name===place.name?"active":""}`} style={{left:`${place.x}%`,top:`${place.y}%`,animationDelay:`${index*60}ms`}} onClick={()=>{if(!mapZoom.consumeDrag()){onSelect(place);setShowDetails(true);}}} onDoubleClick={event=>{event.preventDefault();window.open(place.googleMapsUrl,"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected.name===place.name}><span><PlaceIcon kind={place.kind}/></span><em className="marker-name">{place.name}</em></button>)}
      </div></div>
...
function AllPlacesMap(){
  ...
  const pin=(place:OverviewPlace,compact=false)=>{const position=!compact?udoInsetSafePositions[place.name]??place:place;return <button key={`${isReserve(place)?"reserve":place.dayIndex}-${place.name}`} data-place-id={place.id} className={`map-pin all-map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected===place?"active":""} ${compact?"compact":""}`} style={{left:`${position.x}%`,top:`${position.y}%`}} onFocus={()=>{setSelected(place);setShowDetails(true);}} onClick={()=>{if(!mapZoom.consumeDrag()){setSelected(place);setShowDetails(true);}}} onDoubleClick={event=>{event.preventDefault();window.open(place.googleMapsUrl,"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected===place}><span><PlaceIcon kind={place.kind}/></span><em className="marker-name">{place.name}</em></button>;};
...
export default function Home(){
  ...
  return <main>
    ...
    {view==="summary"&&<><section className="summary-intro"><span>TRIP AT A GLANCE</span><h1>우리의 일주일,<br/>한눈에 보기</h1><p>일정의 대표 장면을 먼저 보고, 마음이 가는 날을 골라보세요.</p></section><section className="overview section-shell" id="overview"><div className="section-heading"><div><span className="section-number">01</span><p>OUR SEVEN DAYS</p></div><h2>일곱 장면으로<br/>미리 보는 제주</h2><p>매일 하나의 좋은 장면만 기억해도 충분한 여행.<br/>카드를 눌러 그날의 자세한 선택지를 확인해요.</p></div>
      <div className="day-grid">{days.map((item,index)=><button key={item.date} className="day-card" onClick={()=>moveToDay(index)} style={{"--day-accent":item.accent} as React.CSSProperties}><div className="day-card-top"><span>DAY {index+1}</span><small>{item.date} {item.weekday}</small></div><img className="day-card-image" src={assetUrl(dayImages[index].src)} alt={dayImages[index].alt} width={960} height={540} loading="lazy" decoding="async"/><strong>{item.title}</strong><p>{item.summary}</p><span className="card-arrow">자세히 보기 →</span></button>)}</div>
    ...
    {view==="day"&&<section className={`detail-section ${activeDay===1?"udo-day":""}`} id="day-detail" style={{"--active-accent":day.accent} as React.CSSProperties}><div className="section-shell"><div className="detail-title"><div><span>DAY {activeDay+1}</span><small>{day.date} · {day.weekday}요일</small></div><p>{day.eyebrow}</p><h2>{day.title}</h2><p className="detail-summary">{day.summary}</p></div>
      ...
      <div className="choices-section"><div className="choices-head"><h3>오늘, 어디로 갈까?</h3><p>카드를 누르면 지도에서 확인하고, ↗ 버튼으로 Google Maps를 열어요.</p></div><div className="choice-grid">{day.places.map(place=><article key={place.name} className={`choice-card ${selectedPlace===place?"selected":""}`} role="button" tabIndex={0} onClick={()=>selectPlaceAndReveal(place)} onKeyDown={event=>{if(event.target!==event.currentTarget)return;if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPlaceAndReveal(place);}}}><span className={`choice-icon kind-${place.kind}`}><PlaceIcon kind={place.kind}/></span><div><small>{kindLabel[place.kind]} {place.rainy&&<span className="rain-label" title="비 오는 날에도 좋아요">☂</span>}</small><strong>{place.name}</strong><p>{place.note}</p></div><a className="external" href={googleMapsUrlForPlace(place.name)} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`${place.name} Google Maps에서 보기`} title="Google Maps에서 보기">↗</a><PlaceFeedback name={place.name}/></article>)}</div></div>
      {dayReserves.length>0&&<div className="choices-section reserve-choices"><div className="choices-head"><h3>일정 근처 예비 후보 <small>{dayReserves.length}곳</small></h3><p>동선이 꼬이거나 기다림이 길 때, 오늘의 권역 안에서만 골라보세요.</p></div><div className="choice-grid">{dayReserves.map(place=><article key={place.id} className={`choice-card reserve ${selectedPlace.name===place.name?"selected":""}`} role="button" tabIndex={0} onClick={()=>selectPlaceAndReveal(place)} onKeyDown={event=>{if(event.target!==event.currentTarget)return;if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPlaceAndReveal(place);}}}><span className={`choice-icon kind-${place.kind}`}><PlaceIcon kind={place.kind}/></span><div><small>예비 · {kindLabel[place.kind]} {place.rainy&&<span className="rain-label" title="비 오는 날에도 좋아요">☂</span>}</small><strong>{place.name}</strong><p>{place.note||"Google 지도에 저장한 일정 인근 후보"}</p></div><a className="external" href={place.googleMapsUrl} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`${place.name} Google Maps에서 보기`} title="Google Maps에서 보기">↗</a><PlaceFeedback name={place.name}/></article>)}</div></div>}
    ...
    <footer><img className="footer-tangerine" src={assetUrl("footer-tangerine-v1.png")} alt="" aria-hidden="true" width={240} height={240} loading="lazy" decoding="async"/><p>잘 먹고, 천천히 걷고, 오래 기억하기.</p><strong>성호 · 세인의 가을 제주</strong><small>2026. 10. 30 — 11. 05</small></footer>
  </main>;
}
