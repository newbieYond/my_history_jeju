  ...
  return <div className="map-card" aria-label={`${day.title} 약도`} ref={mapRef}>
    ...
    <div className="map-stage-shell"><div className={`map-stage ${isUdo?"udo-map":""} ${mapZoom.zoom>1?"zoomed":""}`} ref={mapZoom.viewportRef} onPointerDown={mapZoom.onPointerDown} onPointerMove={mapZoom.onPointerMove} onPointerUp={mapZoom.onPointerUp} onPointerCancel={mapZoom.onPointerCancel}>
      <div className="map-scroll-space" style={{width:`${mapZoom.zoom*100}%`,height:`${mapZoom.zoom*100}%`}}><div className="map-zoom-canvas" style={{width:`${100/mapZoom.zoom}%`,height:`${100/mapZoom.zoom}%`,transform:`scale(${mapZoom.zoom})`,"--map-marker-scale":1/mapZoom.zoom} as CSSProperties}>
        ...
        {places.map((place,index) => <button key={place.id} className={`map-pin pin-${place.kind} ${selected.name===place.name?"active":""}`} style={{left:`${place.x}%`,top:`${place.y}%`,animationDelay:`${index*60}ms`}} onClick={()=>{if(!mapZoom.consumeDrag())onSelect(place);}} onDoubleClick={event=>{event.preventDefault();window.open(mapUrl(place.name),"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected.name===place.name}><span>{kindIcon[place.kind]}</span></button>)}
      </div></div>
...
function AllPlacesMap(){
  ...
  const [visiblePlaceIds,setVisiblePlaceIds]=useState<number[]>(allPlaces.map(place=>place.id));
  const mapZoom=useMapZoom();
  ...
  const mapPlaces: OverviewPlace[] = showReserve?[...allPlaces,...reservePlaces]:allPlaces;
  ...
  const pin=(place:OverviewPlace,compact=false)=><button key={`${isReserve(place)?"reserve":place.dayIndex}-${place.name}`} data-place-id={place.id} className={`map-pin all-map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected===place?"active":""} ${compact?"compact":""}`} style={{left:`${place.x}%`,top:`${place.y}%`}} onFocus={()=>setSelected(place)} onClick={()=>{if(!mapZoom.consumeDrag())setSelected(place);}} onDoubleClick={event=>{event.preventDefault();window.open(place.googleMapsUrl,"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected===place}><span>{isReserve(place)?"+":kindIcon[place.kind]}</span><em>{place.name}</em></button>;
