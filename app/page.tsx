...
function JejuMap({ day, selected, onSelect, mapRef }: { day: Day; selected: Place; onSelect: (place: Place) => void; mapRef: RefObject<HTMLDivElement|null> }) {
  ...
  return <div className="map-card" aria-label={`${day.title} 약도`} ref={mapRef}>
    ...
    <div className={`map-stage ${isUdo?"udo-map":""}`}>
      ...
      {selected && <div className={`map-popover ${popoverPosition(selected)}`} role="status"><span className={`place-kind kind-${selected.kind}`}>{kindLabel[selected.kind]}</span><strong>{selected.name}</strong><p>{selected.note}</p><a href={mapUrl(selected.name)} target="_blank" rel="noreferrer">Google Maps에서 보기 ↗</a></div>}
