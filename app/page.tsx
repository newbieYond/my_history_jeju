...
function AllPlacesMap(){
  ...
  const pin=(place:OverviewPlace,compact=false)=><button key={`${isReserve(place)?"reserve":place.dayIndex}-${place.name}`} data-place-id={place.id} className={`map-pin all-map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected===place?"active":""} ${compact?"compact":""}`} style={{left:`${place.x}%`,top:`${place.y}%`}} onMouseMove={()=>setSelected(place)} onFocus={()=>setSelected(place)} onClick={()=>{if(!mapZoom.consumeDrag())setSelected(place);}} onDoubleClick={event=>{event.preventDefault();window.open(place.googleMapsUrl,"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected===place}><span>{isReserve(place)?"+":kindIcon[place.kind]}</span><em>{place.name}</em></button>;
