...
export default function Home(){
  ...
  return <main>
    ...
    {view==="day"&&<section className={`detail-section ${activeDay===1?"udo-day":""}`} id="day-detail" style={{"--active-accent":day.accent} as React.CSSProperties}><div className="section-shell"><div className="detail-title"><div><span>DAY {activeDay+1}</span><small>{day.date} · {day.weekday}요일</small></div><p>{day.eyebrow}</p><h2>{day.title}</h2><p className="detail-summary">{day.summary}</p></div>
      ...
      <div className="choices-section"><div className="choices-head"><h3>오늘, 어디로 갈까?</h3><p>카드를 누르면 지도에서 확인하고, ↗ 버튼으로 Google Maps를 열어요.</p></div><div className="choice-grid">{day.places.map(place=><article key={place.name} className={`choice-card ${selectedPlace===place?"selected":""}`} role="button" tabIndex={0} onClick={()=>selectPlaceAndReveal(place)} onKeyDown={event=>{if(event.target!==event.currentTarget)return;if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPlaceAndReveal(place);}}}><span className={`choice-icon kind-${place.kind}`}>{kindIcon[place.kind]}</span><div><small>{kindLabel[place.kind]} {place.rainy&&<span className="rain-label" title="비 오는 날에도 좋아요">☂</span>}</small><strong>{place.name}</strong><p>{place.note}</p></div><a className="external" href={googleMapsUrlForPlace(place.name)} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`${place.name} Google Maps에서 보기`} title="Google Maps에서 보기">↗</a></article>)}</div></div>
      {dayReserves.length>0&&<div className="choices-section reserve-choices"><div className="choices-head"><h3>일정 근처 예비 후보 <small>{dayReserves.length}곳</small></h3><p>동선이 꼬이거나 기다림이 길 때, 오늘의 권역 안에서만 골라보세요.</p></div><div className="choice-grid">{dayReserves.map(place=><article key={place.id} className={`choice-card reserve ${selectedPlace.name===place.name?"selected":""}`} role="button" tabIndex={0} onClick={()=>selectPlaceAndReveal(place)} onKeyDown={event=>{if(event.target!==event.currentTarget)return;if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPlaceAndReveal(place);}}}><span className={`choice-icon kind-${place.kind}`}>+</span><div><small>예비 · {kindLabel[place.kind]} {place.rainy&&<span className="rain-label" title="비 오는 날에도 좋아요">☂</span>}</small><strong>{place.name}</strong><p>{place.note||"Google 지도에 저장한 일정 인근 후보"}</p></div><a className="external" href={place.googleMapsUrl} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`${place.name} Google Maps에서 보기`} title="Google Maps에서 보기">↗</a></article>)}</div></div>}
