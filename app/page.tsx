...
function JejuMap({ day, selected, onSelect }: { day: Day; selected: Place; onSelect: (place: Place) => void }) {
  ...
  return <div className="map-card" aria-label={`${day.title} 약도`}>
    ...
    <div className={`map-stage ${isUdo?"udo-map":""}`}>
      ...
      {day.places.map((place,index) => <button key={place.name} className={`map-pin pin-${place.kind} ${selected===place?"active":""}`} style={{left:`${place.x}%`,top:`${place.y}%`,animationDelay:`${index*60}ms`}} onClick={()=>onSelect(place)} aria-label={`${place.name} 정보 보기`}><span>{kindIcon[place.kind]}</span></button>)}
...
function AllPlacesMap(){
  ...
  const pin=(place:OverviewPlace,compact=false)=><button key={`${isReserve(place)?"reserve":place.dayIndex}-${place.name}`} className={`map-pin all-map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected===place?"active":""} ${compact?"compact":""}`} style={{left:`${place.x}%`,top:`${place.y}%`}} onMouseEnter={()=>setSelected(place)} onFocus={()=>setSelected(place)} onClick={()=>setSelected(place)} aria-label={`${place.name} 정보 보기`}><span>{isReserve(place)?"+":kindIcon[place.kind]}</span><em>{place.name}</em></button>;
...
export default function Home(){
  ...
  return <main>
    ...
    {view==="day"&&<section className={`detail-section ${activeDay===1?"udo-day":""}`} id="day-detail" style={{"--active-accent":day.accent} as React.CSSProperties}><div className="section-shell"><div className="detail-title"><div><span>DAY {activeDay+1}</span><small>{day.date} · {day.weekday}요일</small></div><p>{day.eyebrow}</p><h2>{day.title}</h2><p className="detail-summary">{day.summary}</p></div>
      <div className="detail-grid"><JejuMap day={day} selected={selectedPlace} onSelect={selectPlace}/><div className="plan-panel"><div className="panel-title"><span>추천 흐름</span><small>시간은 가볍게 참고만</small></div><div className="schedule-list">{day.schedule.map((item,index)=>{const linkedPlace=day.places.find(place=>place.name===schedulePlaceNames[activeDay][index])??day.places[0];const selectSchedule=()=>{setSelectedScheduleIndex(index);setSelectedPlace(linkedPlace);};return <article key={`${item.time}-${item.title}`} className={selectedScheduleIndex===index?"selected":""} role="button" tabIndex={0} onMouseEnter={selectSchedule} onFocus={selectSchedule} onClick={selectSchedule} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();selectSchedule();}}}><time>{item.time}</time><div><strong>{item.title}</strong><p>{item.note}</p></div>{item.rainy&&<span className="rain-dot" title="비 오는 날에도 좋아요" aria-label="비 오는 날에도 좋아요">☂</span>}</article>})}</div><div className="today-tip"><span>작은 약속</span><p>{day.tip}</p></div></div></div>
      <div className="choices-section"><div className="choices-head"><h3>오늘, 어디로 갈까?</h3><p>카드를 누르면 지도에서 확인하고, 화살표로 길찾기를 열어요.</p></div><div className="choice-grid">{day.places.map(place=><article key={place.name} className={`choice-card ${selectedPlace===place?"selected":""}`} role="button" tabIndex={0} onClick={()=>selectPlace(place)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPlace(place);}}}><span className={`choice-icon kind-${place.kind}`}>{kindIcon[place.kind]}</span><div><small>{kindLabel[place.kind]} {place.rainy&&<span className="rain-label" title="비 오는 날에도 좋아요">☂</span>}</small><strong>{place.name}</strong><p>{place.note}</p></div><a className="external" href={mapUrl(place.name)} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`${place.name} Google Maps에서 보기`}>↗</a></article>)}</div></div>
