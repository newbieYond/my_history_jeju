...
function JejuMap({ day, dayIndex, selected, onSelect, mapRef }: { day: Day; dayIndex: number; selected: Place; onSelect: (place: Place) => void; mapRef: RefObject<HTMLDivElement|null> }) {
  ...
  return <div className="map-card" aria-label={`${day.title} 약도`} ref={mapRef}>
    ...
    </div>{selected&&<div className="map-popover map-popover-detail" role="status"><span className={`place-kind kind-${selected.kind}`}>{kindLabel[selected.kind]}</span><strong>{selected.name}</strong><p>{selected.note}</p><a className="map-photo-link" href={googleMapsUrlForPlace(selected.name)} target="_blank" rel="noreferrer">사진 보기 ↗</a></div>}<MapZoomControls zoom={mapZoom.zoom} onChange={mapZoom.changeZoom}/></div><p className="map-caption">마커를 누르면 장소 정보가 보여요 · 사진 보기로 해당 장소의 Google Maps 사진을 열 수 있어요.</p>
  </div>;
}
...
function AllPlacesMap(){
  ...
  const pin=(place:OverviewPlace,compact=false)=>{const position=!compact?udoInsetSafePositions[place.name]??place:place;return <button key={`${isReserve(place)?"reserve":place.dayIndex}-${place.name}`} data-place-id={place.id} className={`map-pin all-map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected===place?"active":""} ${compact?"compact":""}`} style={{left:`${position.x}%`,top:`${position.y}%`}} onFocus={()=>setSelected(place)} onClick={()=>{if(!mapZoom.consumeDrag())setSelected(place);}} onDoubleClick={event=>{event.preventDefault();window.open(place.googleMapsUrl,"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected===place}><span>{isReserve(place)?"+":kindIcon[place.kind]}</span><em className="marker-name">{place.name}</em></button>;};
  return <section className="all-map-wrap">
    ...
    <div className="map-stage-shell all-map-stage-shell"><div className={`map-stage all-map-stage ${mapZoom.zoom>1?"zoomed":""}`} ref={mapZoom.viewportRef} onPointerDown={mapZoom.onPointerDown} onPointerMove={mapZoom.onPointerMove} onPointerUp={mapZoom.onPointerUp} onPointerCancel={mapZoom.onPointerCancel}><div className="map-scroll-space" style={{width:`${mapZoom.zoom*100}%`,height:`${mapZoom.zoom*100}%`}}><div className="map-zoom-canvas" style={{width:`${100/mapZoom.zoom}%`,height:`${100/mapZoom.zoom}%`,transform:`scale(${mapZoom.zoom})`,"--map-marker-scale":1/mapZoom.zoom} as CSSProperties}><img className="map-background" src={assetUrl("jeju-map-detail-v1.webp")} alt="" aria-hidden="true" width={1536} height={1024} loading="lazy" decoding="async"/>{mainland.map(place=>pin(place))}
      ...
      </div></div></div><div className="map-popover map-popover-detail" role="status"><span className={`place-kind kind-${selected.kind}`}>{isReserve(selected)?"예비 장소":`DAY ${selected.dayIndex+1}`} · {kindLabel[selected.kind]}</span><strong>{selected.name}</strong><p>{selected.note}</p><a className="map-photo-link" href={selected.googleMapsUrl} target="_blank" rel="noreferrer">사진 보기 ↗</a></div><MapZoomControls zoom={mapZoom.zoom} onChange={mapZoom.changeZoom}/></div>
...
export default function Home(){
  ...
  return <main>
    ...
    {view==="day"&&<section className={`detail-section ${activeDay===1?"udo-day":""}`} id="day-detail" style={{"--active-accent":day.accent} as React.CSSProperties}><div className="section-shell"><div className="detail-title"><div><span>DAY {activeDay+1}</span><small>{day.date} · {day.weekday}요일</small></div><p>{day.eyebrow}</p><h2>{day.title}</h2><p className="detail-summary">{day.summary}</p></div>
      ...
      <div className="choices-section"><div className="choices-head"><h3>오늘, 어디로 갈까?</h3><p>카드를 누르면 지도에서 확인하고, 화살표로 길찾기를 열어요.</p></div><div className="choice-grid">{day.places.map(place=><article key={place.name} className={`choice-card ${selectedPlace===place?"selected":""}`} role="button" tabIndex={0} onClick={()=>selectPlaceAndReveal(place)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPlaceAndReveal(place);}}}><span className={`choice-icon kind-${place.kind}`}>{kindIcon[place.kind]}</span><div><small>{kindLabel[place.kind]} {place.rainy&&<span className="rain-label" title="비 오는 날에도 좋아요">☂</span>}</small><strong>{place.name}</strong><p>{place.note}</p></div><a className="external" href={googleMapsUrlForPlace(place.name)} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`${place.name} Google Maps에서 보기`}>↗</a></article>)}</div></div>
