...
const days: Day[] = [
  ...
  },
  {
    ...
    summary: "말차 디저트로 시작해 용머리해안, 사계, 신창, 협재로 이어지는 긴 해안 드라이브.", accent: "#a16d42",
    schedule: [
      ...
      { time: "12:10", title: "사계해안·송악산", note: "용머리해안 입장 불가 시 이 구간을 넉넉히" },
    ...
    ], tip: "용머리해안은 물때·기상에 따른 현장 통제가 잦아요. 입장이 가능하면 송악산 산책은 짧게, 막히면 사계해안으로 바로 전환해요.",
  },
  {
    date: "11.04", weekday: "수", eyebrow: "SWEET SLOW DAY", title: "감귤빛 완충일",
    summary: "직접 딴 감귤 한 봉지와 이중섭거리 산책으로 채우는 서귀포의 느린 오후.", accent: "#df7e36",
    schedule: [
      { time: "오전", title: "감귤 따기", note: "후기 좋은 농장에서 30~60분" },
      { time: "점심", title: "서귀포 향토식", note: "네거리식당 등 가볍게", rainy: true },
      { time: "오후", title: "이중섭거리 산책", note: "카페를 곁들인 필수 도심 코스", rainy: true },
    ...
    ], tip: "감귤 따기와 이중섭거리는 고정으로 두고, 천지연·새연교만 체력과 날씨에 맞춰 조절해요.",
  },
  {
...
const schedulePlaceNames = [
  ...
  ["오설록 티뮤지엄", "오설록 티뮤지엄", "용머리해안", "송악산", "신창풍차해안", "협재·금능"],
  ...
  return <div className="map-card" aria-label={`${day.title} 약도`} ref={mapRef}>
    ...
    <div className="map-stage-shell"><div className={`map-stage ${isUdo?"udo-map":""} ${mapZoom.zoom>1?"zoomed":""}`} ref={mapZoom.viewportRef} onPointerDown={mapZoom.onPointerDown} onPointerMove={mapZoom.onPointerMove} onPointerUp={mapZoom.onPointerUp} onPointerCancel={mapZoom.onPointerCancel}>
      <div className="map-scroll-space" style={{width:`${mapZoom.zoom*100}%`,height:`${mapZoom.zoom*100}%`}}><div className="map-zoom-canvas" style={{width:`${100/mapZoom.zoom}%`,height:`${100/mapZoom.zoom}%`,transform:`scale(${mapZoom.zoom})`,"--map-marker-scale":1/mapZoom.zoom} as CSSProperties}>
        ...
        {places.map((place,index) => <button key={place.id} className={`map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected.name===place.name?"active":""}`} style={{left:`${place.x}%`,top:`${place.y}%`,animationDelay:`${index*60}ms`}} onClick={()=>{if(!mapZoom.consumeDrag())onSelect(place);}} onDoubleClick={event=>{event.preventDefault();window.open(place.googleMapsUrl,"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected.name===place.name}><span>{isReserve(place)?"+":kindIcon[place.kind]}</span></button>)}
      </div></div>
    </div>{selected&&<div className="map-popover map-popover-detail" role="status"><span className={`place-kind kind-${selected.kind}`}>{kindLabel[selected.kind]}</span><strong>{selected.name}</strong><p>{selected.note}</p><a href={mapUrl(selected.name)} target="_blank" rel="noreferrer">Google Maps에서 보기 ↗</a></div>}<MapZoomControls zoom={mapZoom.zoom} onChange={mapZoom.changeZoom}/></div><p className="map-caption">마커를 누르면 장소 정보가 보여요 · 확대 후 PC에서는 휠·끌기, 모바일에서는 끌기로 지도를 이동할 수 있어요</p>
  </div>;
}
...
function AllPlacesMap(){
  ...
  const pin=(place:OverviewPlace,compact=false)=>{const position=!compact?udoInsetSafePositions[place.name]??place:place;return <button key={`${isReserve(place)?"reserve":place.dayIndex}-${place.name}`} data-place-id={place.id} className={`map-pin all-map-pin pin-${place.kind} ${isReserve(place)?"reserve":""} ${selected===place?"active":""} ${compact?"compact":""}`} style={{left:`${position.x}%`,top:`${position.y}%`}} onFocus={()=>setSelected(place)} onClick={()=>{if(!mapZoom.consumeDrag())setSelected(place);}} onDoubleClick={event=>{event.preventDefault();window.open(place.googleMapsUrl,"_blank","noopener,noreferrer");}} aria-label={`${place.name} 정보 보기`} aria-pressed={selected===place}><span>{isReserve(place)?"+":kindIcon[place.kind]}</span><em>{place.name}</em></button>;};
  return <section className="all-map-wrap">
    ...
    <div className="map-stage-shell all-map-stage-shell"><div className={`map-stage all-map-stage ${mapZoom.zoom>1?"zoomed":""}`} ref={mapZoom.viewportRef} onPointerDown={mapZoom.onPointerDown} onPointerMove={mapZoom.onPointerMove} onPointerUp={mapZoom.onPointerUp} onPointerCancel={mapZoom.onPointerCancel}><div className="map-scroll-space" style={{width:`${mapZoom.zoom*100}%`,height:`${mapZoom.zoom*100}%`}}><div className="map-zoom-canvas" style={{width:`${100/mapZoom.zoom}%`,height:`${100/mapZoom.zoom}%`,transform:`scale(${mapZoom.zoom})`,"--map-marker-scale":1/mapZoom.zoom} as CSSProperties}><img className="map-background" src={assetUrl("jeju-map-detail-v1.webp")} alt="" aria-hidden="true" width={1536} height={1024} loading="lazy" decoding="async"/>{mainland.map(place=>pin(place))}
      ...
      </div></div></div><div className="map-popover map-popover-detail" role="status"><span className={`place-kind kind-${selected.kind}`}>{isReserve(selected)?"예비 장소":`DAY ${selected.dayIndex+1}`} · {kindLabel[selected.kind]}</span><strong>{selected.name}</strong><p>{selected.note}</p><a href={mapUrl(selected.name)} target="_blank" rel="noreferrer">Google Maps에서 보기 ↗</a></div><MapZoomControls zoom={mapZoom.zoom} onChange={mapZoom.changeZoom}/></div>
...
export default function Home(){
  ...
  const dayReserves=reservePlaces.filter(place=>place.tags.includes(`day${activeDay+1}`));
  ...
  return <main>
    ...
    {view==="day"&&<section className={`detail-section ${activeDay===1?"udo-day":""}`} id="day-detail" style={{"--active-accent":day.accent} as React.CSSProperties}><div className="section-shell"><div className="detail-title"><div><span>DAY {activeDay+1}</span><small>{day.date} · {day.weekday}요일</small></div><p>{day.eyebrow}</p><h2>{day.title}</h2><p className="detail-summary">{day.summary}</p></div>
      ...
      {dayReserves.length>0&&<div className="choices-section reserve-choices"><div className="choices-head"><h3>일정 근처 예비 후보 <small>{dayReserves.length}곳</small></h3><p>동선이 꼬이거나 기다림이 길 때, 오늘의 권역 안에서만 골라보세요.</p></div><div className="choice-grid">{dayReserves.map(place=><article key={place.id} className={`choice-card reserve ${selectedPlace.name===place.name?"selected":""}`} role="button" tabIndex={0} onClick={()=>selectPlaceAndReveal(place)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPlaceAndReveal(place);}}}><span className={`choice-icon kind-${place.kind}`}>+</span><div><small>예비 · {kindLabel[place.kind]} {place.rainy&&<span className="rain-label" title="비 오는 날에도 좋아요">☂</span>}</small><strong>{place.name}</strong><p>{place.note||"Google 지도에 저장한 일정 인근 후보"}</p></div><a className="external" href={place.googleMapsUrl} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`${place.name} Google Maps에서 보기`}>↗</a></article>)}</div></div>}
